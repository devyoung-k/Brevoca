...
async function mergeChunkSummaries(
  ...
  const text = await requestSummaryText(
    [
      ...
      "Deduplicate semantically overlapping points when they convey the same conclusion or status update.",
      "Within each topic, order points so the key issue, decision, or unresolved risk appears first.",
      "The objective should explain the purpose/background of the meeting in 1-3 sentences.",
      "The conclusions should read like numbered end-of-meeting conclusions, not raw action items or repeated topic bullets.",
      "Keep nextSteps concrete and actionable. Leave unclear assignee or dueDate as null, but omit vague tasks that are not executable.",
...
function buildTranscriptSummaryPrompt(
  ...
  return [
    ...
    "Within each topic, surface the most important issue or decision direction first instead of preserving transcript order.",
    "Compress repeated statements that mean the same thing into one representative sentence, but keep genuinely different viewpoints.",
    "Set conclusions to only the final agreed directions, approved decisions, or conclusion-level pending items that belong under '3. 회의 결론'.",
    "Do not repeat raw topic bullets in conclusions. Conclusions should read like final takeaways, not discussion notes.",
    "Only include nextSteps when there is a concrete executable follow-up task. Write each item as one clear task with a concrete verb.",
    "If assignee, due date, or task target are all unclear, prefer reviewNotes over a vague nextStep.",
    "Items that are only caveats, ASR ambiguity, name confusion, or facts requiring later confirmation should go to reviewNotes, not topics.",
    "Do not drop any distinct discussion point, but remove low-value repetition and compress filler.",
...
function buildChunkSummaryPrompt(
  ...
  return [
    ...
    "Only include nextSteps when this chunk contains a concrete executable follow-up task with a clear action.",
    "Items that are only caveats, ASR ambiguity, name confusion, or facts requiring later confirmation should go to reviewNotes, not topics.",
    "Do not drop any distinct discussion point, but remove low-value repetition and compress filler.",
