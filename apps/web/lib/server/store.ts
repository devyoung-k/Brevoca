...
export function buildMeetingAudioStorageKey(
    meetingId: string,
    fileName: string,
): string {
    return `meetings/${meetingId}/original${getAudioExtension(fileName)}`;
}

export function buildMeetingAudioChunkPrefix(meetingId: string): string {
    return `meetings/${meetingId}/chunks`;
}

export function buildMeetingAudioChunkKey(
    meetingId: string,
    chunkIndex: number,
): string {
    return `${buildMeetingAudioChunkPrefix(meetingId)}/part_${String(chunkIndex).padStart(4, "0")}.part`;
}

function getMeetingAudioStorageKeys(
    meetingId: string,
    fileName: string,
    audioChunkCount: number,
): string[] {
    if (audioChunkCount > 1) {
        return Array.from(
            { length: audioChunkCount },
            (_, index) => buildMeetingAudioChunkKey(meetingId, index),
        );
    }

    return [buildMeetingAudioStorageKey(meetingId, fileName)];
}
...
function mapMeetingDetail(row: MeetingRow): StoredMeeting {
    return {
        ...
        audioChunkCount: row.audio_chunk_count ?? 1,
...
export async function createMeeting(
    ...
    const audioChunkCount = input.audioChunkCount ?? 1;
    const storageKey =
        input.storageKey ??
        buildMeetingAudioStorageKey(meetingId, input.fileName);
    ...
    if (input.fileBuffer) {
        ...
        throw new Error("Missing storageKey for chunked upload.");
    }
        ...
        await requireSingle(
            supabase
                ...
                .insert({
                    ...
                    prompt_template_id:
                        input.promptTemplateId || defaultPromptTemplateId,
                    tags: input.tags,
...
export async function listMeetings(
    ...
    const { data, error } = await supabase
        .from("meetings")
        .select(
            "id, job_id, title, status, source_type, language, duration_sec, created_at, updated_at, prompt_template_id, tags",
        )
...
export async function acquireJobLease(
    ...
    const supabase = getSupabaseAdmin();
    const leaseUntil = new Date(Date.now() + leaseTtlMs).toISOString();
    const { data, error } = await supabase.rpc("acquire_job_lease", {
        p_job_id: jobId,
    ...
    if (error) {
        throw new Error(`Failed to acquire job lease: ${error.message}`);
    }
...
export async function renewJobLease(
    ...
    if (error) {
        throw new Error(`Failed to renew job lease: ${error.message}`);
    }
...
export async function clearJobLease(jobId: string): Promise<void> {
    const supabase = getSupabaseAdmin();
    ...
    if (error) {
        throw new Error(`Failed to clear job lease: ${error.message}`);
    }
}
...
export async function getJobForWorkspace(
    ...
    if (meetingError) {
        throw new Error(
            `Failed to verify job ownership: ${meetingError.message}`,
        );
    }

    if (
        !meetingRow ||
        (meetingRow as { workspace_id: string | null }).workspace_id !==
            workspaceId
    ) {
        return null;
    }
...
export async function requireWorkspaceMembership(
    ...
    if (error) {
        throw new Error(
            `Failed to verify workspace membership: ${error.message}`,
        );
    }
...
export async function getStoredMeetingByJob(
    jobId: string,
...
export async function getStoredMeeting(
    meetingId: string,
    workspaceId: string,
): Promise<StoredMeeting | null> {
    const supabase = getSupabaseAdmin();
...
async function downloadStorageObject(storageKey: string): Promise<Buffer> {
    const supabase = getSupabaseAdmin();
    const bucket = getMeetingAudioBucket();
    ...
    for (
        let attempt = 0;
        attempt <= STORAGE_DOWNLOAD_RETRY_DELAYS_MS.length;
        attempt += 1
    ) {
...
export async function downloadMeetingAudio(
    ...
    if (audioChunkCount <= 1) {
        return downloadStorageObject(storageKey);
    }

    const chunkBuffers: Buffer[] = [];
    for (let chunkIndex = 0; chunkIndex < audioChunkCount; chunkIndex += 1) {
        chunkBuffers.push(
            await downloadStorageObject(
                `${storageKey}/part_${String(chunkIndex).padStart(4, "0")}.part`,
            ),
        );
    }
...
export async function uploadMeetingAudioChunk(
    meetingId: string,
    chunkIndex: number,
    chunkBuffer: Buffer,
): Promise<void> {
    const supabase = getSupabaseAdmin();
    const bucket = getMeetingAudioBucket();
    const storageKey = buildMeetingAudioChunkKey(meetingId, chunkIndex);
    const { error } = await supabase.storage
        .from(bucket)
        .upload(storageKey, chunkBuffer, {
...
export async function removeMeetingAudioChunks(
    meetingId: string,
    audioChunkCount: number,
): Promise<void> {
    ...
    if (error) {
        throw new Error(`Failed to delete meeting audio: ${error.message}`);
    }
}
...
export async function updateMeeting(
    ...
    const payload = {
        ...
        ...(patch.durationSec !== undefined
            ...
            : {}),
        ...(patch.promptTemplateId !== undefined
            ? { prompt_template_id: patch.promptTemplateId }
            : {}),
        ...(patch.tags !== undefined ? { tags: patch.tags } : {}),
        ...(patch.transcriptText !== undefined
            ? { transcript_text: patch.transcriptText }
            : {}),
        ...(patch.transcriptSegments !== undefined
            ? { transcript_segments: patch.transcriptSegments }
            : {}),
...
export async function setProcessingCanceled(
    ...
    try {
        ...
        await updateJob(jobId, {
            ...
            logs: [
                ...job.logs,
                `${message} (canceled 상태 미지원으로 failed 처리)`,
            ],
            errorMessage: null,
            errorType: null,
        });
...
export async function resetJobForRetry(jobId: string): Promise<JobRecord> {
    ...
    if (job.status !== "failed" && job.status !== "canceled") {
        throw new Error("RetryNotAllowed");
    }
...
export async function deleteMeetingForWorkspace(
    meetingId: string,
    workspaceId: string,
): Promise<void> {
    const supabase = getSupabaseAdmin();
    ...
    if (meeting.storageKey) {
        if (meeting.audioChunkCount > 1) {
            await removeMeetingAudioChunks(meeting.id, meeting.audioChunkCount);
        } else {
            const bucket = getMeetingAudioBucket();
            const { error: storageError } = await supabase.storage
                .from(bucket)
                .remove([meeting.storageKey]);
            if (storageError) {
                throw new Error(
                    `Failed to delete meeting audio: ${storageError.message}`,
                );
            }
        }
    }
